...
fn successful_domain_registration() {
    ...
    chain.execute_with(|| {
		...
		let mangled_domain = String::from(DOMAIN_Y);
		let route = String::from("");
		let path = Path::new(&domain, &route).unwrap();
		let mangled_path = Path::new(&mangled_domain, &route).unwrap();
        let owner = ACCOUNT_A;
